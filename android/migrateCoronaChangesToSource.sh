#!/bin/bash
# This script is to port Corona Lab's Facebook SDK changes to the Facebook SDK source code.
...
# This should be run like: ./migrateCoronaChangeToSource.sh 4.5.0 src
...
needToCopyData() {
    if [ -z "$1" ]
    then
        echo "migrateCoronaChangesToSource.needToCopyData needs a resource xml file in the Facebook SDK to compare against"
        exit -1
    else
...
then
    echo ""
	echo "USAGE:  $script old_facebook_sdk_version fb_sdk_src_directory"
	echo -e "\told_facebook_sdk_version: Facebook SDK version we're upgrading from (e.g. 4.5.0)."
	echo -e "\tfb_sdk_src_directory: Name of root directory of Facebook SDK source code."
	exit -1
fi

# 2. Replace SDK number in build.plugin.sh, build.plugin.xml and project.properties
echo "Migrate plugin project files to Facebook SDK source directory."
...
cp -v "$OLD_SDK_PATH/facebook/build.xml" "$NEW_SDK_PATH/facebook/src/main/build.xml"
cp -v "$OLD_SDK_PATH/facebook/project.properties" "$NEW_SDK_PATH/facebook/src/main/project.properties"
# From versioned src code distributions.
...
# libs directory with android-support-v4.jar, android-support-v7-cardview.jar, bolts-android-1.1.2.jar, and BUCK
# For building from Facebook Android SDK Github repo.
if [ -d "$NEW_SDK_PATH/facebook/src/main/libs" ]
then
    echo "Already copied over libs directory with dependencies"
else
    cp -r "$NEW_SDK_PATH/libs" "$NEW_SDK_PATH/facebook/src/main/libs"
fi
# From versioned src code distributions.
...
# For building from Facebook Android SDK Github repo.
PICKER_PATH="$NEW_SDK_PATH/facebook/src/main/java/com/facebook/picker"
# From versioned src code distributions.
...
# For building from Facebook Android SDK Github repo.
SDK_RES_PATH="$NEW_SDK_PATH/facebook/src/main/res"
# From versioned src code distributions.
