		...
		if (mFragmentToLaunch.equals("place")) {
			...
			friendPickerFragment.setOnDoneButtonClickedListener(new PickerFragment.OnDoneButtonClickedListener() {
				...
				public void onDoneButtonClicked(PickerFragment fragment) {
					...
					List<JSONObject> friendsSelection = ((FriendPickerFragment)fragment).getSelection();
	...
	private void pushFriendSelection(final List<JSONObject> friendsSelection) {
		CoronaRuntimeTask task = new CoronaRuntimeTask() {
			...
			private void pushGraphUser(LuaState L, JSONObject graphUser, int index) {
				...
				Log.d("Corona", "FacebookFragmentActivity.pushFriendSelection.pushGraphUser(), graphUser:\n" + graphUser.toString());
			...
			public void executeUsing(CoronaRuntime runtime) {
				if (runtime != null && placeSelection != null && mListener>0) {
					...
					if (L != null) {
						...
						Log.d("Corona", "FacebookFragmentActivity.pushPlaceSelection.executeUsing(), placeSelection:\n" + placeSelection.toString());
						// This is so we can push the proper data to Lua
						try {
							pushStringIfNotNull(L, placeSelection.getString("category"), "category");
		...
		try {
			if(mFragmentToLaunch.equals("friends")) {
				...
				if (bestProvider != null) {
					...
					if (locationManager.isProviderEnabled(bestProvider) && mLocationListener == null) {
